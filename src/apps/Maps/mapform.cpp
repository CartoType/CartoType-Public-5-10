...
void MapForm::RightButtonUp(int32_t aX,int32_t aY)
    ...
    double y[1] = { p.iY };
    int32_t alt[1];
    CartoType::TCoordSet cs(x,y,sizeof(double),1);
...
void MapForm::CalculateAndDisplayRoute()
    {
    CartoType::TCoordSet cs(&m_route_point_array[0].m_point.iX,&m_route_point_array[0].m_point.iY,sizeof(m_route_point_array[0]),m_route_point_array.size());
