...
class TCoordSet
    ...
    TCoordSet(const double* aX,const double* aY,size_t aStride,size_t aCount) noexcept:
        m_x((char*)aX),
        m_y((char*)aY),
        m_stride(aStride),
        m_count(aCount)
    ...
    TCoordSet(const std::vector<TPointFP>& aPointArray) noexcept:
        m_x((char*)&aPointArray.front().iX),
        m_y((char*)&aPointArray.front().iY),
        m_stride(sizeof(TPointFP)),
        m_count(aPointArray.size())
        {
        }

    double X(size_t aIndex) const noexcept { return *(const double*)(m_x + m_stride * aIndex); }
    double Y(size_t aIndex) const noexcept { return *(const double*)(m_y + m_stride * aIndex); }
...
class TWritableCoordSet: public TCoordSet
    {
    public:
    constexpr TWritableCoordSet() noexcept { }
    TWritableCoordSet(double* aX,double* aY,size_t aStride,size_t aCount):
        TCoordSet(aX,aY,aStride,aCount)
        {
        }
    TWritableCoordSet(std::vector<TPointFP>& aPointArray) noexcept:
        TCoordSet(aPointArray)
        {
        }

    double& X(size_t aIndex) const { return *(double*)(m_x + m_stride * aIndex); }
    double& Y(size_t aIndex) const { return *(double*)(m_y + m_stride * aIndex); }
    };
